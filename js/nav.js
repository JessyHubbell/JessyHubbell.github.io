document.addEventListener("DOMContentLoaded", function () {
    ...
    function initializeAccordion(selector = '.accordion-toggle') {
  ...
  toggles.forEach(toggle => {
    ...
    const hotspotPairs = [
    { hotspot: '.bookcase-hotspot', highlight: '.bookcase-hover', link: '', label: 'Bookcase' },
    { hotspot: '.skull-hotspot', highlight: '.skull-hover', link: '', label: 'Skull' }, // link left empty
    { hotspot: '.flower-hotspot', highlight: '.flower-hover', link: '', label: 'Flower' },
    { hotspot: '.crow-hotspot', highlight: '.crow-hover', link: '', label: 'Crow' },
    { hotspot: '.fireplace-hotspot', highlight: '.fireplace-hover', link: '', label: 'Fireplace' },
    { hotspot: '.sigil-hotspot', highlight: '.sigil-hover', link: 'https://open.spotify.com/show/0zFCMVhVlpyykIwCMQckoj?si=d5c2d900d2f64ca2', label: 'Sigil' },
    { hotspot: '.boots-hotspot', highlight: '.boots-hover', link: '', label: 'Boots' },
      { hotspot: '.window-hotspot', highlight: '.window-hover', link: '', label: 'Window' }
  ];

  hotspotPairs.forEach(({ hotspot, highlight, link, label }) => {
  const hot = document.querySelector(hotspot);
  const glow = document.querySelector(highlight);

  console.log(`Checking: ${label}`);
  console.log("  Hotspot element:", hot);
  console.log("  Hover image:", glow);
...
hot.addEventListener('click', (event) => {
  if (label === 'Crow') {
    ...
    event.stopPropagation();   // <-- Just in case it's inside a nested element

    const crowSound = new Audio('sfx/Cawntent.wav');
    crowSound.play();

    crowSound.addEventListener('ended', () => {
      // Optionally navigate after sound ends
      if (link) {
        window.location.href = link;
      }
    });
  } else if (label !== 'Skull' && label !== 'Boots' && label !== 'Window' && label !== 'Flower' && label !== 'Bookcase' && label !== 'Fireplace') {
    window.location.href = link;
  }
});
  }
  ...
  // OVERRIDE ONLY SKULL
  const skullHotspot = document.querySelector('.skull-hotspot');
  if (skullHotspot) {
    skullHotspot.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
...
`, skullHotspot);
    ...
    // Initialize tabs after panel becomes visible
setTimeout(() => {
  initializeAccordion();
}, 50);
      ...
      // OVERRIDE ONLY Bookcase
  const bookcaseHotspot = document.querySelector('.bookcase-hotspot');
  if (bookcaseHotspot) {
    bookcaseHotspot.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
...
setTimeout(() => {
  initializeAccordion();
    ...
    // Boot Override
const bootsHotspot = document.querySelector('.boots-hotspot');
if (bootsHotspot) {
  bootsHotspot.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    // Window Override
const windowHotspot = document.querySelector('.window-hotspot');
if (windowHotspot) {
  windowHotspot.addEventListener('click', (e) => {
    ...
    const flowerHotspot = document.querySelector('.flower-hotspot');
if (flowerHotspot) {
  flowerHotspot.addEventListener('click', (e) => {
    ...
    openMagicPanel(`
      ...
      <div id="flower-tabs">
        <ul>
          <li><a href="#flower-tab-1">Archibald</a></li>
          <li><a href="#flower-tab-2">Reggie</a></li>
          <li><a href="#flower-tab-3">Ursilla</a></li>
          <li><a href="#flower-tab-4">Jeffery</a></li>
          <li><a href="#flower-tab-5">Producer</a></li>
        </ul>
        <div id="flower-tab-1">
    ...
    setTimeout(() => {
      $("#flower-tabs").tabs();
        ...
        // fireplace Override
const fireplaceHotspot = document.querySelector('.fireplace-hotspot');
if (fireplaceHotspot) {
  fireplaceHotspot.addEventListener('click', (e) => {
    e.preventDefault();
    e.stopPropagation();

    openMagicPanel(`
      <h2>The Heist</h2>
      <p><em>Coming Soon.</em> Gods Damn it Jeffery who authorized this...</p>
    `, fireplaceHotspot);
  });
}
});
